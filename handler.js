...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.wm = namabot + ' MUHAMMAD AHYAN NOOR'
             ...
             for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                        let chat = global.db.data.chats[m.chat]
                        let user = global.db.data.users[m.sender]
                        if (!['unbanchat.js', 'dompet.js', 'creator.js'].includes(name) && chat && chat?.isBanned && !isAdmin && !isMods) return // Kecuali ini, bisa digunakan
    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.sendButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'selamat datang' : 'sampai jumpa', action === 'add' ? '.intro' : 'FokusID')
                        }
                    }
                }
                break
            ...
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Sekarang Sudah Tidak Admin```')
    ...
    async groupsUpdate(groupsUpdate, fromMe, m) {
        ...
        for (let groupUpdate of groupsUpdate) {
            ...
            if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || '```Deskripsi Succes Di Ganti Ke```\n@desc').replace('@desc', groupUpdate.desc)
            if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```Judul Berhasil Di ganti Ke```\n@subject').replace('@subject', groupUpdate.subject)
            if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```Icon Berhasil Di Ganti```').replace('@icon', groupUpdate.icon)
            if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```Link Grup Berhasil Di Setel Ulang```\n@revoke').replace('@revoke', groupUpdate.revoke)
            if (groupUpdate.announce == true) text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || '```GROUP BERHASIL DI TUTUP!')
            if (groupUpdate.announce == false) text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || '```GROUP BERHASIL DI BUKA!')
